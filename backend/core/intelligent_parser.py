...
class IntelligentParser:
    ...
    def _parse_sheet(self, xl: pd.ExcelFile, sheet_name: str) -> List[StudentRecord]:
        ...
        sec_match = re.search(r"(?:-|\s)([A-Z])$", sheet_label)
        ...
        for idx in range(len(df)):
            try:
                ...
                # Skip aggregate/summary rows that appear beneath section tables.
                    ...
                    if pd.notna(val):
                        ...
                        if len(text) > 10:
                            ...
                            if not self.test_info.section:
                                sec_match = re.search(r"branch\s*&\s*section\s*:\s*([A-Z0-9()]+)\s*[-:]\s*([A-Z])", text, re.IGNORECASE)
                                if sec_match:
                                    self.test_info.department = (self.test_info.department or sec_match.group(1).upper())
                                    self.test_info.section = sec_match.group(2).upper()
        except Exception:
            pass
