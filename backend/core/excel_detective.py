    ...
    for col_idx in range(len(df.columns)):
        ...
        if confidence > 0.4:
            if best == "marks":
                ...
                skip_headers = (
                    "sl. no", "sl no", "s.no", "s no", "reg", "name", "g / m", "g/m",
                    "d/h", "girl / boy", "girl/boy", "cut off", "cutoff", "unnamed", "nan",
                )
                if any(tok in header_l for tok in skip_headers):
                    continue
...
def _score_marks(col: pd.Series) -> float:
    ...
    if any(tok in header_hint for tok in ("sl", "reg", "name", "g / m", "d/h", "girl", "boy", "cut off")):
        return 0.0
